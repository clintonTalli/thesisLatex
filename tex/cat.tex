...
For every $\epsilon > 0$, there exists an ordered weighted tree $T_\epsilon$ and regular hexagon such that every realization of $T_\epsilon$ as an ordered disk contact graph where the radii of the disks equal the vertex weights, approximates the hexagon such that the Hausdorff distance is less than $\epsilon$.
\end{lem} 
