...
The obstacle hexagon has height of $ 2 s(n,m) \cdot \sqrt{3}$, and the flag is of height $\sqrt{3}$.  
\begin{equation}\label{eqn:Hnm}
H(n,m) = \lr{12s(n,m)+1} 2 s(n,m) \sqrt{3} + 12s(n,m) \lr{\frac{1}{100N} + \sqrt{3}}
\end{equation}

The cross section of the corridor must have a minimum height of $\sqrt{3}$ everywhere.
The height of the $i^\text{th}$ obstacle polygon in noncanonical position is $2 s(n,m) \sec \lr{\alpha_i} \cdot \sqrt{3}$ (see Figure \ref{fig:hexagonNonCanonical.pdf} for reference).%$(t+1) \cdot \sec \alpha_i \cdot \sqrt{3}$.
