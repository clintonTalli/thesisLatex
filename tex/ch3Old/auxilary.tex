...
Let the side length of the an obstacle hexagon be $N(n,m)= c_N \cdot s(n,m)$ where $c_N \in \lr{0, \frac{1}{2z + 1} }$.
...
Scale the honeycomb such that the obstacle hexagons become regular hexagons of side length $(5t-1)/2+\sqrt{3}$, and then scale each obstacle hexagon independently from its center to a hexagon of side length $(5t-1)/2$ (see Figure \ref{fig:ScalingForCorridors.pdf}). 
