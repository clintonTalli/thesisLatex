...
Given a 3-CNF boolean formula, $B$, we define the associated graph as follows: the vertices correspond to the variables and clauses in $B$, when a variable or its negation appears in a clause there is an edge between the corresponding vertices.
...
Problems \ref{prob:Satisfiability-1}---\ref{prob:Satisfiability-2} are known to be NP-hard and are often used to show other problems are NP-hard as well. 
