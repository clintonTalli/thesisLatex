...
Note that with edge definition, it is not possible to have one element subset of $V$ as an edge (sometimes referred to as a self-adjacent edge or loop).
A \textit{simple graph} has no self-adjacent vertices.
Note that in this thesis every graph is a simple graph.
...
In Figure \ref{fig:combinatorialEmbedding.pdf}, the wheel graph is depicted in two different drawings, one on the left and one on the right. 
