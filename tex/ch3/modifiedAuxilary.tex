...
\item Consider a large (polynomial-size) regular hexagon $J$ that contains all gadgets in our construction, and enclose $J$ by a \emph{frame} of 6 congruent regular hexagons, as shown in Fig.~\ref{fig:frame}(a), hinged together in a path.
Denote the distance between any two parallel sides of $J$ as $$H(n,m)=h(n,m) + f(n,m)$$ where $h(n,m)$ is the height defined in the auxilary construction and $f(n,m)$ is a small polynomial to give the modified construction some space between the boundary of $J$ and the perimeter of the construction.
%
...
% In an arbitrary realization of $P$, each obstacle hexagon's position can be 
