...
Denote the distance between any two parallel sides of $R$ as $$H(n,m)=h(n,m) + f(n,m)$$ where $h(n,m)$ is the height polynomial defined in the auxilary construction and $f(n,m)$ is a small polynomial to give the modified construction some space between the boundary of $R$ and the perimeter of the construction.
%
...
% \end{minipage}
%
\item Connect the frame and the obstacles in $R$ into a simply connected polygonal linkage: in each obstacle hexagon, the bottom or bottom-left side is adjacent to the frame or to a corridor. 
