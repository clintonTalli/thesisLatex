...
In Figure \ref{fig:hexagonalConstructionOfJSmallWithoutHalfHexagons.pdf} we show the three smallest possible formal constructions of $J$.  
...
Notice that in each case we six hexagons of the hexagonal grid each have two adjacent lie on the perimeter of $J$.
...
For any $J$, there is a fixed number of hexagons from the hexagonal grid that lie on perimiter; denote this number as $k$.
We can denote the number of hexagons in a row of the hexagonal grid in $J$ with the following sequence as follows:
...
Let the side length of the an obstacle hexagon be $N(n,m)= c_N \cdot s(n,m)$ where $c_N \in (0, .5)$.
The scaling constant $c_N$ preserves the corridors described described earlier.  
%We then finally re-scale this form of the honeycomb such that the side length of the hexagons are determined by polynomial of variable $n$ and $m$, $N(n,m)= c_N \cdot s(n,m)$ where $c_N \in (0, .5)$ is a scaling constant of $s(n,m)$, while preserving the corridors formed from the first scaling.
...
Let $t(n,m)=2N(n,m)^3-1$ be the number of flags in a corridor (see Figure \ref{fig:variable}). 
