...
$$\begin{array}{|c|l|}\label{table:configurationSpace-1}
\text{Object Type}&\text{Definition of Reconfiguration}\\
\text{Graph}&\text{a continuous motion of the vertices that preserves the lengths of the edges 
and never causes the edges to intersect.}\\
\text{Linkage}&\text{Same as graph}\\
\text{Polygonal Linkage}&\text{a continuous motion of polygons that preserves shapes of polygons, 
hinge point pairings, and never causes the polygonal sides to intersect.}\\
\text{Disk Arrangement}&\text{a continuous motion of disks that preserves disk radii, 
pairs of contact points, and never causes disks to intersect.}
\end{array}$$
