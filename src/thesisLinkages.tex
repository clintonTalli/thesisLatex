...
\subsection{Linkages}
%graph -> G = (V,E)
%edge length mapping -> $l: E \mapsto \bbr^+$
%linkage -> (G,l)
%embedding -> $\Pi : V \mapsto \bbR^{2}$
%proper embedding proper -> $l\left( \left(u,v\right) \right) = \left\vert \Pi(u) - \Pi(v) 
%\right\vert$
%realization -> $\Pi(V)$
...
\end{enumerate}
%NEEDS TO SHOW AN EXAMPLE OF A PROPER EMBEDDING
...
%SHOW AN EXAMPLE OF A RIGID MOTION
%SHOW AN EXAMPLE OF AN INVERSION
...
%NEEDS TO SHOW AN EXAMPLE OF A CONTINUOUS CONFIGURATION SPACE

We can extend the properties of graphs with orientation.  
%order with trees, matroids, psuedolines, etc. 
\subsection{Summary}
\begin{table}[!ht]
\begin{center}
$$\begin{array}{|l|c|c|}
